...
public class RPPlayerListener implements Listener {
    ...
    public void onPlayerFrostWalk(EntityBlockFormEvent e) {
        ...
        if (r != null && e.getNewState().getType().name().contains("FROSTED_ICE") && !r.canPlayerIceForm()) {
            e.setCancelled(true);
        }
    }
        ...
        if (itemInHand != null && !itemInHand.getType().equals(Material.AIR)) {
            ...
            if (itemInHand.getType().name().equalsIgnoreCase(RedProtect.get().config.configRoot().wands.adminWandID) && ((claimmode.equalsIgnoreCase("WAND") || claimmode.equalsIgnoreCase("BOTH")) || p.hasPermission("redprotect.admin.claim"))) {
                ...
                if (RedProtect.get().firstLocationSelections.containsKey(p) && RedProtect.get().secondLocationSelections.containsKey(p)) {
                    ...
                    if (loc1.getWorld().equals(loc2.getWorld()) && loc1.distanceSquared(loc2) > RedProtect.get().config.configRoot().region_settings.wand_max_distance && !RedProtect.get().ph.hasPerm(p, "redprotect.bypass.define-max-distance")) {
        ...
        if (r == null) {
            ...
            if (itemInHand != null && (event.getAction().name().equals("RIGHT_CLICK_BLOCK") || b == null)) {
                ...
                if (hand.equals(Material.ENDER_PEARL) && !r.canTeleport(p)) {
                    ...
                    return;
                } else if ((hand.equals(Material.BOW) || (hand.name().contains("SNOW") && hand.name().contains("BALL")) || hand.name().contains("FIREWORK") || hand.equals(Material.EGG)) && !r.canProjectiles(p)) {
                    RedProtect.get().lang.sendMessage(p, "playerlistener.region.cantuse");
                    event.setCancelled(true);
                    event.setUseItemInHand(Event.Result.DENY);
                    return;
                } else if (hand.equals(Material.POTION) && !r.canUsePotions(p)) {
                    RedProtect.get().lang.sendMessage(p, "playerlistener.region.cantuse");
                    event.setCancelled(true);
                    event.setUseItemInHand(Event.Result.DENY);
                    return;
            ...
            if (b != null) {
                if (b.getType().name().endsWith("PRESSURE_PLATE")) {
                    ...
                    if (!r.canFish(p)) {
                        ...
                        b.getType().name().contains("SIGN") && !r.canPlaceSign(p)) {
    ...
    public void onEntityDamageEvent(EntityDamageEvent e) {
        ...
        if (r != null) {
            ...
            if (!r.isDeathAllowed() && play.getHealth() <= 1) {
                e.setCancelled(true);
            }
        }
    ...
    public void onEntityDamageByEntityEvent(EntityDamageByEntityEvent e) {
        ...
        if ((e.getEntity() instanceof Boat || e.getEntity() instanceof Minecart) && !r.canPlaceVehicle(p)) {
